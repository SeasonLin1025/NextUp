...
1. 先按风险层级排（Tier1 最优先）
2. 层级相同，按紧急程度排（high > medium > low）
3. 再按时间余裕排（slack 越小越优先）
4. 再按截止时间排（越早越优先）

因此可能出现这种情况：第二名的余裕更小或截止更近，但因为与第一名同属一个风险层级，而第一名紧急程度更高，所以第一名在前。遇到这种情况要如实说明，例如"两个任务剩余时间都够用，属于同一风险层级，测试111被标记为紧急，所以优先做它"。不要用错误理由搪塞。
